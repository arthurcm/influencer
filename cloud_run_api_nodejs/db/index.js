...
function getUserBalance(influencer_id) {
    // const client = await pool.connect()
    return pool.query('SELECT * FROM influencer_account WHERE influencer_id = $1', [influencer_id]);
}

function getUserTransactionHistory(influencer_id) {
    return pool.query('SELECT * FROM influencer_transaction WHERE influencer_id = $1', [influencer_id]);
}

async function addCampaignPayment(influencer_id, amount, campaign_id) {
    const client = await pool.connect();
    let status = {};
    try {
...
async function cashOutBalance(influencer_id, amount) {
    const client = await pool.connect();
    let status = {};
    try {
        await client.query('BEGIN');
...
async function convertCampaignPayment(influencer_id, convert_date) {
    const client = await pool.connect();
    let status = {};
    try {
        await client.query('BEGIN');
