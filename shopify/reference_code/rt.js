...
const gfp_additional=window.__goaffpro&&window.__goaffpro.identifiers;
const isFirstTouch=window.__goaffpro&&"first_touch"===window.__goaffpro.first_touch_or_last;
...
function getShop(){
    let t=void 0;
    return Object.keys(document.scripts)
        .forEach(function (o){const e=document.scripts[o];e.src&&e.src.startsWith("https://static.goaffpro.com/reftracker.js")&&(t=e.src.replace("https://static.goaffpro.com/reftracker.js?shop=",""));
        }),t||window.__goaffpro&&window.__goaffpro.shop||document.location.host;
}
...
function trackVisit(){
    ...
    if(o||t){
        ...
        r.send(JSON.stringify({
            ...
            referrer:document.referrer,discount_code:t,
        })),
        r.onload=function (){
            if(420===r.status)
            {deleteCookie("ref"),
            ...
            {try{
                ...
                if(o.discount_code&&!t){
                    ...
                    o.apply_discount_on_myshopify?e.open("GET",`https://${o.website}/discount/${encodeURIComponent(o.discount_code)}`):e.open("GET",`/discount/${encodeURIComponent(o.discount_code)}`),
                    e.send();
                }
            }
            catch(o){}}},
...
function setCookie(o,e){
    ...
    if(t){
        const r=`expires=${new Date((new Date).getTime()+1e3*t).toUTCString()}`;
...
function gfp_remove_cookies(){
    ...
    deleteCookie("source"),
    deleteCookie("discount_code"),
    deleteCookie("sitestripe");
}
gfp_additional&&gfp_additional.forEach(function (o){
    goaffpro_identifiers.push(o);
});
let refcode=getRefCode();
refcode&&setCookie("ref",refcode);
...
function updateSiteLink(){
    ...
    if(e){
        ...
        if(t&&0!==t.length){
            t.forEach(
                function (o){
                    o.href&&(-1<o.href.search(e)||o.host!==document.location.host)&&(o.href=function (o,e,t){e=encodeURIComponent(e);
                        const r=document.createElement("a");
                        return e+=t?`=${encodeURIComponent(t)}`:"",r.href=o,r.search+=(r.search?"&":"")+e,r.href;}(o.href,"ref",refcode));
                });
        }
    }
}
...
function showSiteStripeToolbar(){
    if(!gfp_aff_toolbar&&window.__goaffpro.aff_bar_config){
        const o=searchInQuery(["toolbar"],document.location.search)||getCookie("sitestripe");
        o&&(gfp_aff_toolbar=window.__goaffpro.aff_bar_config,gfp_aff_toolbar.affiliate_portal_link=`https://${window.__goaffpro.partner_portal_subdomain}`,gfp_aff_toolbar.ref_code=o,loadScript("https://static.goaffpro.com/sitestripe2.js"),
        setCookie("sitestripe",o));
    }
}
source&&setCookie("source",source),"noref"===refcode&&gfp_remove_cookies(),trackVisit(),checkoutPageCallback(),(refcode=getCookie("ref"))&&"organic"!==refcode&&updateSiteLink(),showSiteStripeToolbar();
